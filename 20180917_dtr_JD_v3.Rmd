...
```{r cars}
dat_std<-read.csv('v2_jingdongStdBase.csv')
...
datt1<-select(dat,MobDr1to6_od30,initial_amt_w,amt_apply_w,fluctuate_amt_w,cms_final_amt_w,assessment,atv,businessType,cash,credit,gmv_amt,gmv_rstd,income,income_before_wash,limit,limit_before_loan_fluct,loan_amt,loan_balance,loan_cnt,loan_quality,max_active_days,punish_normal_amt,punish_normal_cnt,refund_amt,weight_account,weight_fluct,weight_time)
...
#library(devtools)
#library(woe)          
IV<-iv.mult(datt1,"MobDr1to6_od30",TRUE)   #原理是以Y作为被解释变量，其他作为解释变量，建立决策树模型
iv.plot.summary(IV)
...
#library(ROSE)
# 过采样&下采样
table(datt1$MobDr1to6_od30)
data_balanced_both <- ovun.sample(MobDr1to6_od30 ~ ., data = datt1, method = "both", p=0.5,N=1000,seed = 1)$data
table(data_balanced_both$MobDr1to6_od30)
...
fit<-(MobDr1to6_od30~.)
rtree<-rpart(fit,minsplit=100, cp=0.03,data=train)
printcp(rtree)
...
table(pre_train,train$MobDr1to6_od30)

#检验test集预测效果#
pre_test<-predict(rtree, newdata = test)
table(pre_test, test$MobDr1to6_od30)

#检验整体集预测效果#
pre_dat<-predict(rtree, newdata = datt1)
table(pre_dat, datt1$MobDr1to6_od30)
```
...
```{r}
result=datt1
result$true_label=result$MobDr1to6_od30
...
#require(caret)
xtab<-table(result$pre_to1,result$MobDr1to6_od30)
...
## roc曲线及AUC
#library(pROC)
modelroc <- roc(datt1$MobDr1to6_od30,pre_dat)
...
table(pre_train_2,train_2$MobDr1to6_od30)

#检验test集预测效果#
pre_test_2<-predict(rtree, newdata = test_2)
table(pre_test_2, test_2$MobDr1to6_od30)

modelroc <- roc(train_2$MobDr1to6_od30,pre_train_2)
plot(modelroc, print.auc=TRUE, auc.polygon=TRUE, grid=c(0.1, 0.2),
     grid.col=c("green", "red"), max.auc.polygon=TRUE,
     auc.polygon.col="skyblue", print.thres=TRUE)

modelroc <- roc(test_2$MobDr1to6_od30,pre_test_2)
plot(modelroc, print.auc=TRUE, auc.polygon=TRUE, grid=c(0.1, 0.2),
     grid.col=c("green", "red"), max.auc.polygon=TRUE,
     auc.polygon.col="skyblue", print.thres=TRUE)
```
