...
fit<-(MobDr1to6_od15~.)
rtree<-rpart(fit,minsplit=200, cp=0.03,data=train)
printcp(rtree)
...
table(pre_dat, datt1$MobDr1to6_od15)
```
...
## 混肴矩阵
result$pre_to1<-ifelse(result$pre_prob>=0.4,1,0)
#require(caret)
...
## roc曲线及AUC
#library(pROC)
modelroc <- roc(datt1$MobDr1to6_od15,pre_dat)
...
table(pre_train_2,train_2$MobDr1to6_od15)

#检验test集预测效果#
pre_test_2<-predict(rtree, newdata = test_2)
table(pre_test_2, test_2$MobDr1to6_od15)

modelroc <- roc(train_2$MobDr1to6_od15,pre_train_2)
plot(modelroc, print.auc=TRUE, auc.polygon=TRUE, grid=c(0.1, 0.2),
     grid.col=c("green", "red"), max.auc.polygon=TRUE,
     auc.polygon.col="skyblue", print.thres=TRUE)

modelroc <- roc(test_2$MobDr1to6_od15,pre_test_2)
plot(modelroc, print.auc=TRUE, auc.polygon=TRUE, grid=c(0.1, 0.2),
     grid.col=c("green", "red"), max.auc.polygon=TRUE,
     auc.polygon.col="skyblue", print.thres=TRUE)
```
